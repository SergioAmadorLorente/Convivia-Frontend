...
export const editarTarea = async (plantillaId: string, data: any, instanceId?: string) => {
    try {
        ...
        const isSingleTask = !data.diasRepeticion || data.diasRepeticion.length === 0;
        ...
        const hasRepetition = Array.isArray(data.diasRepeticion) && data.diasRepeticion.length > 0;
        ...
        if (hasRepetition) {
            ...
            try {
                console.log(`� Obteniendo instancias de plantilla ${plantillaId} para edición masiva...`);
                // Endpoint sugerido por usuario: GET /api/espacios/{espacioid}/tareas/filter?plantillaId={id}
                const filterRes = await api.get(`/espacios/${espacioId}/tareas/filter`, {
                    params: { plantillaId: plantillaId }
                });
                const instancias = filterRes.data; // Asumimos array

                if (Array.isArray(instancias)) {
                    console.log(`🔄 Actualizando ${instancias.length} instancias manualmente...`);
                    ...
                    for (const inst of instancias) {
                        ...
                        try {
                            ...
                            console.warn(`⚠️ Error actualizando instancia ${inst.id}`, errPatch);
                        }
                    }
                }
            } catch (errFilter) {
                console.warn("⚠️ Error obteniendo/filtrando instancias de plantilla:", errFilter);
            }
...
export const obtenerTareasPorEspacio = async (espacioId: string) => {
    try {
        ...
        if (error.response && error.response.status === 404) {
            console.warn("⚠️ Endpoint anidado no encontrado (404). Intentando fallback /Tareas...");
            try {
...
export const obtenerDetallePlantilla = async (espacioId: string, plantillaId: string) => {
    try {
        ...
        if (response.status === 404) {
            console.log(`ℹ️ Plantilla ${plantillaId} no encontrada (404). Posiblemente eliminada.`);
            return null;
        }
        ...
        console.warn(`⚠️ Error al obtener detalle de plantilla ${plantillaId}`, error);
        return null;
    }
}
...
export const obtenerDetalleTareaInstancia = async (espacioId: string, plantillaId: string, tareaId: string) => {
    try {
        ...
        console.warn(`⚠️ Error al obtener detalle de instancia tarea ${tareaId}`, error);
        return null;
    }
}
